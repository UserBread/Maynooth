...
public class SortCustomers {
    ...
    // Bubble Sort by first name (alphabetically)
    ...
    // Bubble Sort by firstname, then surname
    public static void bubbleSortBySurnameThenAge(Customer[] arr) {
        ...
        for (int i = 0; i < n - 1; i++) {
            for (int j = 0; j < n - i - 1; j++) {
                ...
                int firstNameCompare = arr[j].name.compareToIgnoreCase(arr[j + 1].name);
                if (firstNameCompare > 0) {
                    swap = true;
                } 
                // If first names are equal, compare by surname
                else if (firstNameCompare == 0 && arr[j].surname.compareToIgnoreCase(arr[j + 1].surname) > 0) {
                    swap = true;
                }
